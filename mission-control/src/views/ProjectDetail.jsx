...
export default function ProjectDetail({ project, onBack }) {
  ...
  function buildKickoffPrompt() {
    return [
      ...
      `**Max iterations:** ${project.max_iterations || 10}`,
      ``,
      `## How to run this loop (critical — follow exactly)`,
      ``,
      `You are the orchestrator. Keep your own context minimal. All heavy work goes to One.`,
      ``,
      `**Each iteration, make a SINGLE call to collective__one with this structure:**`,
      ``,
      `- task: Full iteration instructions — tell One to: (1) research the topic with web searches, (2) write or revise the document, (3) score it 1-10 for clarity/completeness/persuasiveness, (4) identify the single weakest section`,
      `- context: The current draft (if iteration > 1), previous score, and what to improve`,
      ``,
      `**One will return a compact result containing:** score, the full revised draft, and the weakest section.`,
      ``,
      `**You then:**`,
      `1. Record the score and decision (KEEP if score improved, REVERT if not)`,
      `2. Reply to the user with: iteration number, score, decision, one sentence on what changed`,
      `3. If score < 9 and iterations remain, call collective__one again for the next iteration`,
      `4. Stop when score ≥ 9 or after ${project.max_iterations || 10} iterations`,
      `5. Deliver the final document text in your last message`,
      ``,
      `**Critical rules:**`,
      `- Do NOT do research yourself. Do NOT call web_search. Do NOT write files. One does all of that.`,
      `- Do NOT include raw tool output in your replies — only your short summary.`,
      `- Each collective__one call is independent with its own context. Keep your messages to the user brief.`,
    ].join('\n')
  }
